# Chặng 2 — Chọn các Solution Options (20 phút)

**Case:** AI Support Radar (Case C)  
**Điểm xuất phát:** Hypothesis Problem ở `01-evidence-huddle.md`.  
**Mục tiêu gate:** Ba đến bốn option cùng user, situation, task và desired outcome; khác nhau có ý nghĩa ở mechanism/cách phân chia công việc Human–AI.

> **Lưu ý:** Day 17 chỉ có Solution Parking Lot, không có solution ranking được participant xác nhận. Các option dưới đây là **solution hypotheses** để tạo micro-prototype, không phải lời hứa tính năng hay conclusion từ interview.

## 1. Giữ nguyên quyết định chung cho A/B/C/D

| Thành phần | Quyết định chung cho A/B/C/D |
| --- | --- |
...
## 2. Bốn cách giải (Options A, B, C, D)

| Thành phần khác biệt | **Option A — Evidence-backed triage digest** | **Option B — Coach-approved check-in** | **Option C — Common-issue cluster view** | **Option D — Learner-initiated help brief** |
| --- | --- | --- | --- | --- |
| **Solution mechanism** | AI tổng hợp nhiều signal có ngữ cảnh thành evidence card theo từng learner/case để Lab Coach review và chọn hành động. | AI soạn một check-in ngắn, dựa trên issue/context đã được learner chia sẻ; Lab Coach duyệt/chỉnh trước khi gửi. | AI nhóm các issue/error được learner opt-in chia sẻ thành cụm lỗi chung; Lab Coach review rồi chọn broadcast hoặc follow-up riêng. | Learner tự khởi tạo một support request có cấu trúc; AI giúp tổ chức/tóm tắt thông tin đã được learner cung cấp hoặc đồng ý đính kèm để Lab Coach review. |
| **User làm gì?** | Lab Coach đọc nguồn, timeline và phần thiếu; chọn check-in, follow-up, broadcast, defer hoặc dismiss. | Lab Coach chọn recipient, sửa giọng điệu/nội dung, gửi hoặc bỏ qua. Learner có thể trả lời, bỏ qua hoặc opt-out. | Lab Coach xem cụm, kiểm tra ví dụ/source, chọn broadcast hướng dẫn chung, hỏi thêm, tạo follow-up riêng hoặc bỏ qua. | **Learner:** chọn loại hỗ trợ, viết/sửa help brief, quyết định dữ liệu nào được gửi và xác nhận gửi hoặc tự tiếp tục. **Lab Coach:** review request và chọn cách phản hồi. |
| **AI làm gì?** | Tóm tắt/nhóm evidence, nêu uncertainty và đề xuất **các** action khả dĩ; không tự ưu tiên hay gửi hỗ trợ. | Tạo bản nháp có thể chỉnh sửa, gợi ý một câu hỏi làm rõ; không tự gửi, không tự escalation. | Phát hiện điểm chung từ issue/error text được phép dùng, nêu số lượng/mức không chắc chắn và draft nội dung broadcast; không tự broadcast. | Gợi ý câu hỏi làm rõ, tóm tắt symptom–điều đã thử–mục tiêu hỗ trợ và tạo bản nháp brief; không tự đính kèm dữ liệu, gửi request hay xếp ưu tiên. |
| **Trigger** | Lab Coach chủ động mở digest khi bắt đầu checkpoint hoặc khi có request/signal mới; không có auto-alert từ một signal đơn lẻ. | Lab Coach khởi tạo check-in từ một case đã có context, hoặc sau learner tự báo “cần hỗ trợ”; không tự nhắn chỉ vì learner im lặng. | Lab Coach mở khi nhiều learner opt-in báo lỗi/issue tương tự hoặc khi Lab Coach muốn review checkpoint theo chủ đề. | Chỉ do **learner chủ động chọn “Cần hỗ trợ”**; không trigger từ im lặng, dwell time, AI/search use hoặc suy luận hành vi. |
| **Trade-off chính** | Evidence minh bạch và hỗ trợ triage tốt, nhưng card có thể nhiều thông tin và vẫn tốn thời gian review. | Giảm rào cản tiếp cận với learner ngại hỏi, nhưng tạo thêm thông điệp/notification và cần bảo vệ quyền không phản hồi. | Hỗ trợ lỗi lặp lại, tiết kiệm capacity, nhưng có nguy cơ làm mờ nhu cầu cá nhân hoặc gom sai các lỗi chỉ trông giống nhau. | Autonomy/privacy rất cao và Lab Coach nhận context rõ ràng, nhưng không tự phát hiện silent struggle của learner chưa chủ động bấm hỗ trợ. |
...
Learner chủ động bấm **“Cần hỗ trợ”**, chọn loại hỗ trợ mong muốn và mô tả ngắn: lỗi/khái niệm đang kẹt, điều đã thử, và checkpoint liên quan. AI chỉ hỗ trợ biến thông tin đó thành một **help brief** rõ ràng. Learner xem, sửa, chọn dữ liệu đính kèm và xác nhận gửi; Lab Coach nhận một request có context rồi tự quyết định check-in, hỗ trợ 1:1, hướng dẫn chung, ticket hoặc defer.
- **Giá trị cần test:** Learner có cảm thấy việc gửi request dễ hơn so với viết một câu hỏi tự do/đợi Lab Coach không? Lab Coach có nhận được đủ bối cảnh để giảm số lượt hỏi lại không?
- **Rủi ro cần test:** Form có khiến learner thấy phải cung cấp quá nhiều thông tin hoặc bỏ cuộc trước khi gửi không? Learner có hiểu AI chỉ dựa trên dữ liệu họ chọn chia sẻ không?

## 4. Distance check giữa các option (A, B, C, D)
...
- **C khác D:** C xử lý pattern lỗi của nhiều learner; D giữ ngữ cảnh và quyền chọn dữ liệu đính kèm của **một learner**. C ưu tiên broadcast/cluster, D ưu tiên request hỗ trợ có cấu trúc.
- **D khác A & B:** A bắt đầu từ evidence mà Lab Coach review, B là check-in từ Lab Coach tới learner; D bắt đầu từ **request do learner chủ động tạo** tới Lab Coach. D trao trọn quyền khởi tạo và chọn đính kèm dữ liệu cho learner.

> Không có option nào chỉ khác label, layout hoặc wording. Cả bốn cùng giải quyết một task, nhưng khác mechanism, trigger, phân công Human–AI và trade-off.
